  ...
  index: (_req, res) => {
    ...
    Client.find()
      ...
      .catch((err) => {
        // Obsługa błędów
        console.error(
          "Błąd podczas przetwarzania zapytania do bazy danych:",
          err
        );
        if (res && res.status) {
          //wersja gpt
          ...
          res
            .status(500)
            .json({
              error: "Wystąpił błąd podczas przetwarzania zapytania: ${err}",
            });
        } else {
          console.error(
            "Obiekt res lub jego metoda status jest niezdefiniowany."
          );
        }
      });
  },
  ...
  create: (req, res) => {
    ...
    clientNew
      ...
      .catch((err) => {
        ...
        // res.send(err);
        // res.status(500).json({ error: 'Nieprawidłowe dane JSON w body' });
        // oprocz obiektu z polem error ,dodaje pole blad: z obiektem systemowym err
        res.json({ error: 'Nieprawidłowe dane JSON w body', blad: err });
      });
  },
  ...
  update: (req, res) => {
    ...
    Client.findByIdAndUpdate(req.params.id, req.body, { new: true })
      ...
      .catch((err) => {
        ...
        res
          .status(500)
          .json({ error: "Nieprawidłowe dane JSON w body", blad: err });
      });
  },
